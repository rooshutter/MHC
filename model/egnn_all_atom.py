...
class GCL(nn.Module):
    ...
    def forward(self, h, edge_index, edge_attr=None, node_attr=None, node_mask=None, edge_mask=None):
        ...
        if node_mask is not None:
            h = h * node_mask
        # normalized_angle = self.norm_angle(h[:, :14].reshape(-1, 7, 2)).reshape(-1, 14)
        # h = torch.cat([normalized_angle, h[:, 14:]], dim=1)
        return h, mij
...
class EquivariantUpdate(nn.Module):
    def __init__(self, hidden_nf, normalization_factor, aggregation_method,
        ...
        self.rot_mlp = nn.Sequential(
            nn.Linear(input_rot, hidden_nf), #edges_in_d 131 -> 163
            act_fn,
            nn.Linear(hidden_nf, hidden_nf),
            act_fn,
        ...
        self.angle_mlp = nn.Sequential(
            ...
            nn.Linear(hidden_nf, hidden_nf),
            act_fn,
            nn.Linear(hidden_nf, 14, bias=False)
        )
    ...
    def forward(self, h, rot, x, angles, edge_index, coord_diff, coord_cross,
        ...
        rot = self.rot_model(h, rot, edge_index, coord_diff, coord_cross,
