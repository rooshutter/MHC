...
from ReQFlow.so3_utils import quaternion_slerp_exp, calc_quat_wt_qt_q1
...
class Flow_Matching_Model_all_atom(nn.Module):
    ...
    def forward(self, z_data):
        ...
        # print(f"{v_hat_mol.shape=}")
        # print(f"{v_hat_mol[:9][:, :7]=}")
    ...
    def compute_flow_match(self, z_data, t_is_0 = False):
        ...
        if rot_vec:
            quat = T_peptide[:,:,:4]
            print(f"{quat.shape=}")
            quat = quat.view(-1, quat.shape[-1])
            rot = quat_to_matrix_torch(quat)
            print(f"{rot.shape=}")
            rot = rot.view(T_peptide.shape[0], T_peptide.shape[1], -1)
            trans = T_peptide[:,:,4:]
            T_peptide = torch.cat((rot, trans), dim=-1)
        ...
        rotquats_t = quaternion_slerp_exp(t.squeeze(-1).expand(-1, T_peptide.shape[1]), T_peptide[:,:,:4], rotquats_0) 


        # T_peptide_t = (1 - t) * T_peptide + t * T_peptide_z
        T_peptide_t = t * T_peptide + (1 - t) * T_peptide_z
        T_peptide_t[:,:,:4] = rotquats_t
    ...
    def train_loss(
        ...
        # x_hat_mol, v_hat_mol = self.predict_pos(molecule, v_hat_mol[:,:7])
        x_hat_mol = self.predict_pos(molecule, v_hat_mol[:,:7], v_hat_mol[:,7:7+14])

        v_x_mol = v_x_mol.reshape(-1, v_x_mol.shape[-1])
        v_hat_mol_quat = v_hat_mol[:,:4]
        v_hat_mol_quat = v_hat_mol_quat.reshape(-1, z_t_mol.shape[1], v_hat_mol_quat.shape[-1])
        v_hat_mol_trans = v_hat_mol[:,4:7]
        v_hat_mol_angles = v_hat_mol[:,7:7+14]
        print(f"{v_hat_mol_angles[0]=}")
        print(f"{molecule['torsion_angles_sin_cos'][0][0][0]=}")
        ...
        # print(f"{gt_rot_quat_vf.shape=}")
        # print(f"{pred_rots_quat_vf.shape=}")
        # print(f"{gt_rot_quat_vf[0]=}")
        # print(f"{pred_rots_quat_vf[0]=}")

        #reshape to 288, 4
        # x1 = x1.reshape(-1, x1.shape[-1])
        # distance = SphereManifold().distance(v_hat_mol_quat, x1)
        distance = gt_rot_quat_vf - pred_rots_quat_vf 
        ...
        # print(f"{atom14_exists.shape=}")
        x_alt_mol = molecule["atom14_alt_gt_positions"]
        # print(f"{x_alt_mol.shape=}")
        ...
        # Atom-distance RMSE (A). For per-coordinate RMSE, divide denominator by 3 additionally.
        rmse = torch.sqrt(error_x / n_atoms_per_graph)
        ...
        print(f"Position RMSE mean: {rmse.mean()}")
        ...
        # print(f"{x1_pos.shape=}")
        # print(f"{x1_pos[0:9]=}")
        # print(f"{v_hat_mol_trans[0:9]=}")
        rmse = torch.sqrt(error_trans / molecule['size'])
        # print(f"Translation RMSE: {rmse}")
        print(f"Translation RMSE mean: {rmse.mean()}")

        angles = molecule['torsion_angles_sin_cos']
        angles = angles.reshape(-1, angles.shape[-2]*angles.shape[-1])
    ...
    def sample_structure(self, num_samples, molecule, protein_pocket, sampling_without_noise, data_dir, run_id):
        ...
        if solver == "loop":
            ...
            ode_func = ODEWrapper(
                self, T_peptide, xh_pro, molecule, molecule_pos, protein_pocket, step_size=(1.0 / self.T)
            )
        ...
        angles_hat = current_xh_mol[:,7:7+14]
        ...
        # x_hat_mol, T_peptide_hat = self.predict_pos(molecule, T_peptide_hat)
        x_hat_mol = self.predict_pos(molecule, T_peptide_hat, angles_hat)
        # print(f"{T_peptide_hat[0]=}")
...
class ODEWrapper(nn.Module):
    def __init__(self, model, T_peptide, xh_pro, molecule, molecule_pos, protein_pocket, step_size):
        super().__init__()
        self.model = model
    ...
    def forward(self, t, xh_mol):
        ...
        v_x_trans = (v_x[:, 4:7] - xh_mol[:, 4:7]) * scale
        ...
        scaling = 10
        # print(f"{self.step_size=}, {scale=}")
        v_x_quat = quaternion_slerp_exp(scaling * self.step_size, v_x[:, :4], xh_mol[:, :4])
        v_x_quat = (v_x_quat - xh_mol[:, :4]) * scale
        # v_x_quat = (v_x_quat - self.T_peptide[:, :4]) * scale

        v_x = torch.cat([v_x_quat, v_x_trans], dim=-1)
