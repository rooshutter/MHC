...
def skew_matrix_exponential_map(
    ...
    # Get standard terms.
    # Use safe angles for the division branch to avoid NaN gradients at zero
    mask_zero = torch.abs(angles) < tol
    angles_safe = torch.where(mask_zero, torch.ones_like(angles), angles)
    angles_sq_safe = torch.where(mask_zero, torch.ones_like(angles_sq), angles_sq)
    
    sin_coeff = torch.where(mask_zero, 1.0 - angles_sq / 6.0, torch.sin(angles_safe) / angles_safe)
    cos_coeff = torch.where(mask_zero, 0.5 - angles_sq / 24.0, (1.0 - torch.cos(angles_safe)) / angles_sq_safe)
...
def rotvec_to_rotmat(rotation_vectors: torch.Tensor, tol: float = 1e-7) -> torch.Tensor:
    ...
    # Use sqrt(sum + eps) instead of torch.norm to avoid NaN gradient at zero vectors.
    # torch.norm gradient = v/||v|| which is 0/0 = NaN at v=0.
    # This is needed because masked-out nodes always have zero rotation vectors.
    rotation_angles = torch.sqrt(torch.sum(rotation_vectors ** 2, dim=-1) + 1e-12)
...
def rotmat_to_rotvec(rotation_matrices: torch.Tensor) -> torch.Tensor:
    ...
    # Use gradient-safe sqrt to avoid NaN at zero diagonal elements.
    vector_pi = torch.sqrt(torch.relu(torch.diagonal(skew_outer, dim1=-2, dim2=-1)) + 1e-12)
    ...
    # Use gradient-safe norm.
    signs_line_idx = torch.argmax(torch.sqrt(torch.sum(skew_outer**2, dim=-1) + 1e-12), dim=-1).long()
...
def angle_from_rotmat(
    ...
    # Use gradient-safe norm to avoid NaN at identity matrix (skew_vectors = 0)
    angles_sin = torch.sqrt(torch.sum(skew_vectors**2, dim=-1) + 1e-12) / 2.0
...
def _rotquat_to_axis_angle(
    ...
    rotation_axes_norms = torch.sqrt(torch.sum(rotation_axes**2, dim=-1) + 1e-12)
...
class BaseSampleSO3(nn.Module):
    ...
    def sample_vector(self, num_sigma: int, num_samples: int) -> torch.Tensor:
        ...
        vectors = vectors / (torch.sqrt(torch.sum(vectors**2, dim=2, keepdim=True) + 1e-12))
        return vectors
...
def quaternion_slerp_exp(t: torch.Tensor, q1: torch.Tensor, q0: torch.Tensor):
    ...
    # Use gradient-safe norm
    q0 = q0 / torch.sqrt(torch.sum(q0**2, dim=-1, keepdim=True) + 1e-12)
    q1 = q1 / torch.sqrt(torch.sum(q1**2, dim=-1, keepdim=True) + 1e-12)
    ...
    # Slightly larger epsilon for acos gradient stability
    epsilon = 1e-6
    ...
    qt_rel = qt_rel / torch.sqrt(torch.sum(qt_rel**2, dim=-1, keepdim=True) + 1e-12)  # Normalize to ensure unit quaternion
    ...
    qt = qt / torch.sqrt(torch.sum(qt**2, dim=-1, keepdim=True) + 1e-12)  # Shape: [B, N, 4]
...
def calc_quat_wt_qt_q1(qt: torch.Tensor, q1: torch.Tensor, epsilon: float = 1e-6) -> torch.Tensor:
    ...
    qt = qt / torch.sqrt(torch.sum(qt**2, dim=-1, keepdim=True) + 1e-12)
    q1 = q1 / torch.sqrt(torch.sum(q1**2, dim=-1, keepdim=True) + 1e-12)
